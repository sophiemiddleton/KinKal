...
namespace KinKal {
  ...
  LHelix::LHelix( Vec4 const& pos0, Mom4 const& mom0, int charge, double bnom, TRange const& range) : LHelix(pos0,mom0,charge,Vec3(0.0,0.0,bnom),range) {std::cout<<" helix 1 "<<endl;}
  ...
  LHelix::LHelix( PDATA const& pdata, LHelix const& other) : LHelix(other) {
    pars_ = pdata;
    trange_ = other.range();
    std::cout<<" Initiatizling"<< trange_<<std::endl;
  }

  /*LHelix::LHelix(LHelix const& other) :  trange_(other.trange_),pars_(other.pars_), mass_(other.mass_), charge_(other.charge_), bnom_(other.bnom_){}*/
