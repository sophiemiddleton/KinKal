...
int main(int argc, char **argv) {
  ...
  Mom4 momv(mom*sint*sin(phi),mom*sint*cos(phi),mom*cost,pmass);//TODO - check
  KTRAJ lhel(origin,momv,icharge,bnom);
  TFile tpfile("TPocaKTLine.root","RECREATE");
  ...
  for(unsigned itime=0;itime < ntstep;itime++){
    ...
    // the line traj must be perp. to this and perp to the track TODO ---> from below
